...
} from "@/redux/listings/inactive/inactiveApiSlice";
...
const ListingItem = ({ listing }: ListingProps) => {
  ...
  const [activateListing, { isLoading: isDeactivating }] =
    useActivateListingMutation();
    ...
    >
      ...
      <View className="flex-row justify-between">
        <IconButton
          ...
          size={12}
          onPress={() => activateListing(listing._id)}
        />
...
const InactiveListings = () => {
  ...
  const [activeTab, setActiveTab] = useState<string | JSX.Element>("Inactive");
  ...
  const {
    ...
    { itemsPerPage, page, filters: { bedrooms } },
    {
